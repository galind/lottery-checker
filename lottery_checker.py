...
def fetch_lottery_data(numero, fecha):
    ...
    try:
        ...
        result_elements = soup.find_all(['div', 'span', 'p'], class_=lambda x: x and any(word in x.lower() for word in ['resultado', 'premio', 'numero', 'loteria']))
        if result_elements:
